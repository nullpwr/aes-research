...
void state_shift_rows(Round * r) {
    uint8_t t, t1;
    t = r->state.as8[1];
    r->state.as8[1] = r->state.as8[5];
    r->state.as8[5] = r->state.as8[9];
    r->state.as8[9] = r->state.as8[13];
    r->state.as8[13] = t;
    ///----
    t = r->state.as8[2];
    r->state.as8[2] = r->state.as8[10];
    r->state.as8[10] = t;
    t = r->state.as8[6];
    r->state.as8[6] = r->state.as8[14];
    r->state.as8[14] = t;
    t = r->state.as8[3];
    r->state.as8[3] = r->state.as8[15];
    t1 = r->state.as8[7];
    r->state.as8[7] = t;
    r->state.as8[15] = r->state.as8[11];
    r->state.as8[11] = t1;
}
...
void state_mix_colums(Round * r) {
    for (int i = 0; i < 4; i++) {
        ...
        //mix_column(&s->encoded.encoded8[i*4]);
    }
}
...
void rkey_next_round(Round * r) {
    //RoundKey RotWord
    r->tempv.as32[0] = r->rkey.as32[3];


    //RoundKey (temp) RotWord
    r->tempv.as32[0] = rorv32(r->tempv.as32[0], 8);
...
void rcon_next_round(Round * r) {
    if (r->rcon.as8[0] == 0) { //TEST idx can be [3] (BEndian)
        r->rcon.as8[0] = 0x1;
        return;
    }
    r->tempv.as32[0] = r->rcon.as8[0] << 1;
    if (r->rcon.as8[0] & 0x80) {
        r->tempv.as8[0] ^= 0x1B;
    }
    r->rcon.as8[0] = r->tempv.as8[0];
    round_reset_tempv(r);
}

void round_init(Round* r) {
    //r = malloc(sizeof(Round));
    //if (r) {
        r->step = -1;
        r->path = LOAD;
        r->rcon.as32 = 0;
...
void loadRound(Round* r) {
    ...
    //ALARMA Logic was changed (sbox build from 0..255)
    // see `aes_sbox_init` line+2
    //Set first byte (0x63) required by FIPS197 standard
    r->tempv.as8[0] = AES_SBOX_FIRST_BYTE;
}

//Arm64 doesn't have ROL instruction
...
static
void printRound(uint8_t arr[], size_t size) {
...
//NOTE Strange bus error when malloc
//malloc free
...
int main(void) {
    ...
    do {
        round.next(&round);
        printf("\n\nSBOX (look 0x63 index only)");
        printRound(round.box.sbox[AES_SBOX_FIRST_BYTE], AES_SBOX_SIZE);
        printf("\n\ninvSBOX (look inv(0x63) index only");
        printRound(round.box.ibox[AES_SBOX_FIRST_BYTE], AES_SBOX_SIZE);
        printf("\n\nSBOX (look 0x60 index only)");
        printRound(round.box.sbox[60], AES_SBOX_SIZE);
        printf("\n\ninvSBOX (look inv(0x60) index only");
        printRound(round.box.ibox[60], AES_SBOX_SIZE);
    } while (round.step < 0); //Pseudo break. See nextRound()
    return 0;
}
